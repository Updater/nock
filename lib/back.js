...
const record = {
  ...
  start: function (fixture, options) {
    ...
    if(!(options.host || isDisabledRecordingFor(options))){
      if (!context.isLoaded) {
        recorder.record({
          dont_print: true,
          output_objects: true,
...
function isDisabledRecordingFor(options) {
  ...
  const disabled = disablerecordingFor && disablerecordingFor.test(options.host)
  debug('Back recording ', disabled ? '' : 'not', 'disabled for', options.host)
  return disabled
}
